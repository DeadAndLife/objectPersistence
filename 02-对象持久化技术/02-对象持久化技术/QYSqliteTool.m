...
- (void)deleteCertain:(NSString*)deleteName{
    //1.打开数据库
    if ([self openDB]){
    //2.编写sql语句
    NSString *sql = @"select * from students where name is ?";
    //3.sql语句转化为预编译对象
    //3.1声明预编译对象
        sqlite3_stmt *stmt;
    //3.2转化预编译对象
        sqlite3_prepare_v2(db, [sql UTF8String], -1, &stmt, NULL);
    //3.3bind参数
        sqlite3_bind_text(stmt, 1, [deleteName UTF8String], -1, NULL);
    //4.执行预编译对象
        while (sqlite3_step(stmt)==SQLITE_ROW) {
            //从预编译对象中取出每一列的值,赋值给mode
            QYStudent *mode=[self extractModebyStmt:stmt];
            [self deleteModelFromColumn:mode];
        }
    }
}

- (BOOL)deleteModelFromColumn:(QYStudent*)mode{
    //1.打开数据库
    if(![self openDB])return NO;
    //2.编写sql语句
    NSString *name = mode.name;
    NSString *sql = @"delete from students where name is ?";
    //3.sql语句转化为预编译对象
    //3.1声明预编译对象
    sqlite3_stmt *stmt;
    //3.2转化预编译对象
